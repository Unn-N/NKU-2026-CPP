...
DemoWidget::~DemoWidget() { qDeleteAll(itemList); }

// ── 槽位 ─────────────────────────────────────────────
void DemoWidget::initSlots()
{
    slotPositions.clear();
    for (int i = 0; i < BAR_COUNT; i++)
...
// ── 模式切换 ─────────────────────────────────────────
void DemoWidget::switchMode(Mode m)
{
    resetBars();
    currentMode = m;
...
// ── 颜色 ─────────────────────────────────────────────
QColor DemoWidget::blendColor(qreal t) const
{
...
// ── 动画 ─────────────────────────────────────────────
void DemoWidget::animateSlideTo(Rectitem* item, qreal targetX, int duration)
{
...
void DemoWidget::animateColorTo(Rectitem* item, qreal targetBlend, int duration)
{
...
// ── 鼠标移动 ─────────────────────────────────────────
int DemoWidget::targetIndexForDrag()
{
...
void DemoWidget::rearrangeOthers()
{
...
// ── 鼠标释放 ─────────────────────────────────────────
void DemoWidget::mouseReleaseEvent(QMouseEvent *event)
{
...
void DemoWidget::triggerVictory()
{
    isSorted    = true;
    algoRunning = false;
    stepTimer->stop();
...
// ── 冒泡排序 ─────────────────────────────────────────
void DemoWidget::buildBubbleSteps()
{
    algoSteps.clear();
    QList<Rectitem*> arr = itemList;
    int n = arr.size();

    QList<Rectitem*> doneBars;

    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - 1 - i; j++) {
            Rectitem* a = arr[j];
            Rectitem* b = arr[j+1];
            QList<Rectitem*> currentDone = doneBars;
            algoSteps.append([this, a, b, currentDone]() {
                for (auto* bar : itemList) {
                    animateColorTo(bar, currentDone.contains(bar) ? 0.5 : 0.0, 80);
                }
        ...
        doneBars.append(done);
        algoSteps.append([this, done]() { animateColorTo(done, 0.5, 200); });
    }
    doneBars.append(arr[0]);
    algoSteps.append([this]() {
        for (auto* bar : itemList) animateColorTo(bar, 0.5, 200);
    });
}

// ── 选择排序 ─────────────────────────────────────────
void DemoWidget::buildSelectionSteps()
{
    algoSteps.clear();
    int n = itemList.size();
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
            ...
            algoSteps.append([this, scanBarIdx, curMinBarIdx, currentDone]() {
                for (auto* bar : itemList) {
                    animateColorTo(bar, currentDone.contains(bar) ? 0.5 : 0.0, 60);
                }
        ...
        if (minSlot != i) {
            ...
            int targetA = minSlot;
            int targetB = i;
            algoSteps.append([this, barA, barB, targetA, targetB, currentDone]() {
                for (auto* bar : itemList) {
                    animateColorTo(bar, currentDone.contains(bar) ? 0.5 : 0.0, 60);
                }
        ...
        Rectitem* doneBar = itemList[slot[i]];
        doneBars.append(doneBar);
        QList<Rectitem*> updatedDone = doneBars;
        algoSteps.append([this, updatedDone]() {
            for (auto* bar : itemList) {
                animateColorTo(bar, updatedDone.contains(bar) ? 0.5 : 0.0, 200);
            }
        });
    }

    doneBars.append(itemList[slot[n - 1]]);
    algoSteps.append([this]() {
        for (auto* bar : itemList) animateColorTo(bar, 0.5, 200);
    });
...
// ── 插入排序 ─────────────────────────────────────────
void DemoWidget::buildInsertionSteps()
{
    algoSteps.clear();
    QList<Rectitem*> arr = itemList;
    int n = arr.size();
    ...
    for (int i = 1; i < n; i++) {
        ...
        QList<Rectitem*> currentDone = doneBars;
        algoSteps.append([this, key, currentDone]() {
            for (auto* bar : itemList) {
                animateColorTo(bar, currentDone.contains(bar) ? 0.5 : 0.0, 80);
            }
        ...
        doneBars.append(key);
        QList<Rectitem*> updatedDone = doneBars;
        algoSteps.append([this, key, insertSlot, updatedDone]() {
